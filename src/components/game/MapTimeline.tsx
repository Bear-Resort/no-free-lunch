        ...
        {game.variant.revealAfter.map((after, i) =>
          i < revealed ? (
            <div
              key={i}
              className={cn(
                "relative rounded-sm border border-ink/20 bg-[#f8edcf] p-2 shadow-[0_5px_12px_rgba(58,44,26,0.28)] animate-in fade-in zoom-in-95 duration-300",
                TILTS[i % TILTS.length],
              )}
            >
              <span className="absolute -top-2 left-1/2 h-4 w-9 -translate-x-1/2 rotate-2 bg-[#e7d7a9]/80 shadow-sm" />
              <MiniGrid bb={game.maps[i]} cellSize={6} activeClass="bg-ink/85" />
